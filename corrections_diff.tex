...
%DIF DEL new/root.tex           Wed Mar 23 13:47:52 2022
%DIF ADD corrections/root.tex   Sun Apr  3 22:34:10 2022
...
\setdate{23}{03}{2022} % Set date with 3 arguments: {day}{month}{year}.
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  \todo{Ihr Text hier.}
\DIFaddbegin \DIFadd{Moderne Applikationen sind zunehmend von kurzen Antwortzeiten, maßgeschneiderter Hardware und dynamischer Skalierung abhängig.
...
\DIFadd{Diese zeigen, dass unpassende Platzierung und Implementierung von Load Balancern für die schlechte Leistung verantwortlich sind, da diese nicht für Edge Computing angepasst sind, und diese darüber entscheiden welchen Pfad eine Anfrage durch das Netzwerk nehmen muss.
...
\DIFadd{Unsere Evaluierungen zeigen, dass unsere Lösung verglichen mit dem aktuellen Standard, nämlich zentralisiertem Round Robin Load Balancing, abhängig vom Szenario die durchschnittlichen Antwortzeiten um 30\% bis 69\% reduziert.
...
Serverless computing, a potential solution to this, is a computing paradigm that abstracts away the underlying infrastructure, alleviating developers from dealing with the associated complexity\DIFdelbegin \DIFdel{.
Current }\DIFdelend \DIFaddbegin \DIFadd{, but existing }\DIFaddend serverless frameworks are \DIFdelbegin \DIFdel{built for the cloud and its predictable and homogeneous environment, making adaptations necessary to successfully use it at the edge .
Numerous }\DIFdelend \DIFaddbegin \DIFadd{not build for the unique requirements of edge computing.
To address this, numerous }\DIFaddend research prototypes have been built, but under many conditions, and particularly \DIFaddbegin \DIFadd{for }\DIFaddend network bound workloads, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{which are workloads where network transfers make up the majority of the total processing time, the }\DIFaddend performance is still lacking.
In this thesis, we present an approach that changes the \DIFdelbegin \DIFdel{functioning }\DIFdelend \DIFaddbegin \DIFadd{inner working }\DIFaddend of serverless frameworks and existing research prototypes in a way that drastically improves the performance of network bound workloads.
\DIFaddbegin \DIFadd{To find out what causes poor performance with network bound workloads we conduct some preliminary experimentation.
}\DIFaddend 

\DIFdelbegin \DIFdel{Initial }\DIFdelend \DIFaddbegin \DIFadd{These initial }\DIFaddend experiments show that inefficient load balancer placement and load balancing decisions are one \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{among }\DIFaddend the biggest performance problems since those parts of the system \DIFdelbegin \DIFdel{aren't edge optimized.
Thus we }\DIFdelend \DIFaddbegin \DIFadd{are not edge optimized, and their placement and decision making determines each request's path through the network.
We thus }\DIFaddend propose that these components be adapted to better meet the needs of serverless edge computing.
To this end, we develop a \DIFdelbegin \DIFdel{load balancing scheme that assigns weights to nodes according to their performance}\DIFdelend \DIFaddbegin \DIFadd{version of weighted round robin load balancing}\DIFaddend . Since the performance is not known a priori we \DIFdelbegin \DIFdel{use observed response times }\DIFdelend \DIFaddbegin \DIFadd{observe response times during runtime }\DIFaddend as an encompassing black-box-metric, based on which we continuously derive \DIFdelbegin \DIFdel{current }\DIFdelend \DIFaddbegin \DIFadd{appropriate }\DIFaddend weights.
...
In traditional, and even in modern containerized computing environments, developers are required to explicitly specify how their application should be deployed \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{shiPromiseEdgeComputing2016}}\hskip0pt%DIFAUXCMD
}\DIFaddend , thus requiring a great deal of effort and high level of competency. %todo cite this
...
Edge Computing does, however, have its own range of specific challenges that need to be overcome to make use of it effectively\DIFaddbegin \DIFadd{, such as easy programmability by software developers\mbox{%DIFAUXCMD
\cite{shiPromiseEdgeComputing2016} }\hskip0pt%DIFAUXCMD
and service management across its heterogeneous environment}\DIFaddend \cite{shiEdgeComputingVisionChallenges2016}.
...
For this reason, popular serverless platforms such as kubeless\cite{kubeless} and OpenFaaS\cite{openfaas} are built on top of Kubernetes, the \DIFdelbegin \DIFdel{far and away most common }\DIFdelend \DIFaddbegin \DIFadd{de-facto standard }\DIFaddend container orchestration platform.
...
While function placement has been studied more extensively, efficient processing of network bound workloads, \DIFdelbegin \DIFdel{meaning }\DIFdelend \DIFaddbegin \DIFadd{which are }\DIFaddend workloads where network transfers \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{make up }\DIFaddend the dominant part of the overall response time, remains a challenge. For serverless frameworks like OpenFaaS, one of the likely causes is the centralized architecture of it and the underlying container orchestration framework, as outlined by Rausch et al.\cite{skippy}.
...
The ingress points first accept the request and forward it to a \DIFdelbegin \DIFdel{function replicas}\DIFdelend \DIFaddbegin \DIFadd{running instance of the function}\DIFaddend , thus acting as a load balancer for incoming requests.

\begin{figure}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\linewidth]{graphics/graphs/initial_eval_pdfs_3x3.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{graphics/graphs/initial_global_trt.png}
    \DIFaddendFL \caption{Kernel Density Estimate of \DIFdelbeginFL \DIFdelFL{a single }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL experiment run. \DIFdelbeginFL \DIFdelFL{Shown data can be interpreted as probability density functions. Data is visualized for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Result shows }\DIFaddendFL \gls{trt} \DIFdelbeginFL \DIFdelFL{, \mbox{%DIFAUXCMD
\gls{fet} }\hskip0pt%DIFAUXCMD
and the time incurred through network transfers}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of different load balancer implementations in a globally distributed scenario}\DIFaddendFL .}
    \DIFdelbeginFL %DIFDELCMD < \label{fig:initial_eval_pdfs}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:initial_global_trt}
...
This preliminary analysis shows an improvement in mean response time of between 81.3\% and 606.9\%, depending on the scenario. \DIFdelbegin \DIFdel{Fig \ref{fig:initial_eval_pdfs}
}\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:initial_global_trt}
}\DIFaddend shows the fitted probability density functions of the overall response time \DIFdelbegin \DIFdel{, function execution time, and network transfer time in different scenarios.
From this data we can see }\DIFdelend \DIFaddbegin \DIFadd{in a globally distributed scenario. While we can see in Figure \ref{fig:initial_global_trt} that the usage of a more sophisticated load balancing scheme than round robin does improve the performance, as long as the load balancers are centralized the improvement is not particularly significant.
...
\DIFadd{From the combined results of the initial evaluation we learn }\DIFaddend that:
\begin{enumerate}
    ...
    \DIFdelbegin %DIFDELCMD < \iitem %%%
\DIFdelend \DIFaddbegin \item \DIFaddend The location of load balancers has a significant impact on performance, making the effective placement of load balancers important
\end{enumerate}
...
\DIFaddbegin \section{\DIFadd{Structure}}
\DIFadd{Chapter 2 provides relevant background information for the technological environment this work is embedded in. In particular it outlines the concept of serverless computing, serverless edge computing, as well as describing the concepts of load balancing and service placement.
...
\DIFaddend % Target: 12 pages total: 10ish text, 2ish figures
\chapter{Background}
...
Serverless computing \DIFdelbegin \DIFdel{, often also referred to as \mbox{%DIFAUXCMD
\gls{faas}}\hskip0pt%DIFAUXCMD
, }\DIFdelend emerged as a new computing paradigm in the context of cloud computing\DIFdelbegin \DIFdel{.
It }\DIFdelend \DIFaddbegin \DIFadd{, which delegates infrastructure provisioning and configuration to the cloud provider to alleviate software developers from that burden.
\mbox{%DIFAUXCMD
\gls{faas} }\hskip0pt%DIFAUXCMD
is one of the most prominent types of serverless computing products on offer.
It allows developers to specify serverless functions, which are then deployed and scaled by the cloud provider.
It }\DIFaddend is a way in which software developers architect, develop, and deploy applications that is dramatically different from more traditional approaches.
...
Although microservice architectures often rely on the \gls{iaas} \DIFaddbegin \DIFadd{or \mbox{%DIFAUXCMD
\gls{paas} }\hskip0pt%DIFAUXCMD
}\DIFaddend solutions cloud providers offer, and thus abstract away the underlying infrastructure to a certain degree, developers still need to handle most scaling and application specific \gls{qos} requirements themselves.
...
Where microservices and containerized deployments first partitioned a large application into several smaller applications, serverless \DIFdelbegin \DIFdel{takes this even further}\DIFdelend \DIFaddbegin \DIFadd{computing continues this trend of division into smaller components}\DIFaddend , since in serverless computing applications are partitioned into individual \textit{functions}, which \DIFdelbegin \DIFdel{all }\DIFdelend \DIFaddbegin \DIFadd{each }\DIFaddend perform a single action\cite{khandelwalTaureauDeconstructingServerless2020}.

\begin{figure}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{graphics/diagrams/monolith_micro_serverless.drawio.png}
%DIFDELCMD <     %%%
...
While these advantages of serverless computing over more traditional approaches are compelling, there are also idiosyncrasies of serverless computing that \DIFaddbegin \DIFadd{can}\DIFaddend , depending on the application, be problematic.
...
\DIFaddbegin \DIFadd{Each Gateway is thus effectively a load balancer for the serverless functions.
}\DIFaddend Figure \ref{fig:openfaas-gateway-diagram} shows a diagram of the architecture, as it is found in their official documentation\cite{openfaas-gateway}.
...
\begin{figure}
    ...
    \caption{An example of how in \DIFaddbeginFL \DIFaddFL{a) cloud computing all computation is centralized, while in }\DIFaddendFL b) edge computing there are nodes interspersed throughout the network and close to clients\DIFdelbeginFL \DIFdelFL{, while in a) cloud computing all computation is centralized}\DIFdelendFL .}
...
A core concept of this visualization is the capability of resources to \textit{stretch} or \textit{compress}. This means increasing or decreasing width or height of the resource, while doing the inverse to the other side. The condition is that the total area of the resource, i.e. the product of width and height, must be consistent throughout this process. This is effectively the way in which performance regression is modeled in this visualization, but one should \DIFdelbegin \DIFdel{not }\DIFdelend \DIFaddbegin \DIFadd{note }\DIFaddend that this way only works under the condition that performance regression is linear with respect to the load. Luckily this is close to observed real world behaviour, %todo cite this
meaning that at least for understanding the underlying problem this visualization holds.
There are, however, limits to this stretching or compressing resources. From a visual perspective there is a bounding box around each resource, which cannot be exceeded in either height or width. This corresponds to the real world behaviour of workloads not getting faster beyond a certain point \DIFdelbegin \DIFdel{, }\DIFdelend even if resources would be available \DIFdelbegin \DIFdel{one one }\DIFdelend \DIFaddbegin \DIFadd{on one }\DIFaddend side, and requests simply timing out once \DIFdelbegin \DIFdel{responses }\DIFdelend \DIFaddbegin \DIFadd{response times }\DIFaddend exceed a certain \DIFdelbegin \DIFdel{time limit }\DIFdelend \DIFaddbegin \DIFadd{limit on the other}\DIFaddend .
\begin{figure}
    \centering
...
Since \DIFdelbegin \DIFdel{this basic idea }\DIFdelend \DIFaddbegin \DIFadd{the idea of using a black box metric alone }\DIFaddend does not constitute a concrete and testable approach we need to define the implementation details \DIFaddbegin \DIFadd{of how this concept can be applied in practise}\DIFaddend .

A well-known and simple implementation of this \DIFdelbegin \DIFdel{idea }\DIFdelend \DIFaddbegin \DIFadd{concept }\DIFaddend is the \textit{least response time} method of load balancing.
In this approach the server with the lowest average response time gets chosen whenever a request needs to be processed.
While this certainly works in general\DIFaddbegin \DIFadd{, and implicitly solves the proximity issue from Figure \ref{fig:efficient_path}, }\DIFaddend there are a few issues with this approach when it comes to serverless edge computing.
...
\DIFaddbegin \DIFadd{This chapter describes the experiments we conduct as well as their results.
...
\DIFaddend \section{Initial Assessment}
% got 5,5-6 pages
...
\end{tabular}
\caption{\DIFdelbeginFL \DIFdelFL{Mean }\DIFdelendFL  \DIFaddbeginFL \DIFaddFL{Percentage improvement in mean }\DIFaddendFL values of a single experimental run of the initial evaluation in different scenarios. Displayed \DIFaddbeginFL \DIFaddFL{mean }\DIFaddendFL values are in order: \DIFdelbeginFL \DIFdelFL{Total response time}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\gls{trt}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{function execution time}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\gls{fet}}\hskip0pt%DIFAUXCMD
...
\end{tabular}
\caption{50th percentile (i.e. median) values of a single experimental run of the initial evaluation in different scenarios. Displayed values are in order: \DIFdelbeginFL \DIFdelFL{Total response time}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\gls{trt}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{function execution time}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\gls{fet}}\hskip0pt%DIFAUXCMD
}\DIFaddendFL , network time between client and load balancer, and network time between load balancer and function replica}
...
\DIFaddend Since the experiments feature a significant degree of random sampling in their simulation \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{as function location, \mbox{%DIFAUXCMD
\gls{fet} }\hskip0pt%DIFAUXCMD
sampling and client positions are random, }\DIFaddend we ran each experiment 10 times.
...
\DIFaddbegin \DIFadd{Next, we explore the effect the load balancer implementation and parametrization have.
...
}\DIFaddend \subsection{Least Response Time Load Balancing Implementation}
% got 2,5 pages
...
\begin{itemize}
    ...
    \item \textbf{Adapted Classic:} \DIFdelbegin \DIFdel{the same implementation as classic , but it does not reset its internal state when weights are updated }\DIFdelend \DIFaddbegin \DIFadd{an adaptation from the classic implementation, which allows for weights to be updated on the fly without the algorithm having to be restarted}\DIFaddend .
...
\DIFaddbegin \DIFadd{After load balancer implementation and parametrization have been evaluated, we now move on to load balancer resource usage and scale.
...
}\DIFaddend \subsection{Load Balancer Resource Usage}
...
\DIFaddbegin \DIFadd{The results of this experiment are closely related to the pressure threshold for load balancer scaling in our approach.
...
\DIFaddend \section{Osmotic Scaling and Scheduling}
\DIFaddbegin 

\DIFadd{Finally, we evaluate our osmotic scaling and scheduling approach.
...
Of particular interest is how our approach affects the scaling decisions of functions when simulating the default scaling behaviour of serverless frameworks like OpenFaaS\mbox{%DIFAUXCMD
...
\DIFaddend \subsection{Performance of Osmotic Scaling and Load Balancing}
% 2 pages
...
While originally load balancers are only spawned in one city, because all requests originate from it, once requests start coming from another city\DIFaddbegin \DIFadd{, }\DIFaddend another load balancer instance is deployed in that city\DIFaddbegin \DIFadd{, as can be seen around timestamps 00:12 and 00:25 in Figure \ref{fig:osmotic_dynamic_lb_replicas}}\DIFaddend .
...
\DIFdelbegin \DIFdel{Somewhat unexpectedly, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend resource usage between different device types differs significantly \DIFaddbegin \DIFadd{more than one would intuit with the load balancer consuming more than triple the memory on some devices than it does others}\DIFaddend .
